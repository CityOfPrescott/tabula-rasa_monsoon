<?php
/**
 * Template Name: GIS - Property Information Test
 *
 */
get_header(); 
?>
...
//Set up form variables
		...
		if (isset($_POST['street'])) { $street = ' AND street LIKE \'%'. $_POST['street'].'%\''; }
		if (isset($_POST['number'])) { $number = ' AND  ( num_new LIKE \'%'. $_POST['number'].'%\' OR num_old LIKE \'%'. $_POST['number'].'%\' )'; }
		...
		if (isset($_POST['project_name_submitted'])) {
			// Query the db
			$query = 'SELECT * FROM asbuiltsCdoc' . $name . '' .  $street . '' . $number . '' . $filter . '' . ' ORDER BY num_old ASC';
		...
		?>
		<div class="asbuilt_results" onclick="window.open('http://borax/gisscans/as-builts/<?php echo $row['num_old']; ?>.pdf')">
			<div class="numbers">
				<ul>
